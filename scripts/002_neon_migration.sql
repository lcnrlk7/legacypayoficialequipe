...
-- Migração do Supabase para Neon (sem dependência de auth.users)

-- Tabela de perfis de usuários (independente de auth.users)
...
-- Tabela de cobranças PIX (QR Code)
...
-- Tabela de integrações do usuário
...
INSERT INTO system_settings (key, value, description) VALUES
  ('pix_fee_percentage', '"1.5"', 'Taxa percentual para transações PIX'),
  ('min_withdrawal', '"10.00"', 'Valor mínimo para saque'),
  ('max_withdrawal', '"50000.00"', 'Valor máximo para saque por transação'),
  ('daily_withdrawal_limit', '"100000.00"', 'Limite diário de saque'),
  ('kyc_required', '"true"', 'KYC obrigatório para usar o sistema')
